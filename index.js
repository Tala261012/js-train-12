...
function createDictionary(entries) {
  ...
  // const dictionary = new Map(entries); //*так тоже работает
...
function getKeysStartingWith(dictionary, letter) {
  ...
  for (const [key, value] of dictionary) {
    if (key.at(0) === letter) arr[arr.length] = key;
  }
...
function addKeyValuePairs(dictionary, entries) {
  ...
  entries.forEach((elem) => {
    if (dictionary.has(elem[0])) rejected++;
    else {
...
function transformDictionary(dictionary) {
  ...
  for (let elem of dictionary.keys()) keys.push(elem);
  for (let elem of dictionary.values()) values.push(elem);
  for (let elem of dictionary.entries()) entries.push(elem);
...
function checkPresence(dictionary, keys) {
  ...
  keys.forEach((elem) => {
    if (dictionary.has(elem)) presence[presence.length] = true;
    else presence[presence.length] = false;
  });
...
function convertSetToDictionary(set) {
  ...
  set.forEach((elem) => map.set(elem, elem.codePointAt(0)));
